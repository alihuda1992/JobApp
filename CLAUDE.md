...
**Keep-alive**: `.github/workflows/keep-alive.yml` runs daily and calls the `keep_alive()` RPC (migration 007), which upserts a single heartbeat row — a real write, because the original anon `profiles?limit=1` read returned an empty 200 under RLS and Supabase paused the project anyway (Sep 2026). This prevents Supabase free-tier auto-pause. It also pushes an empty heartbeat commit if the repo has had no commits for 45+ days (prevents GitHub's 60-day scheduled-workflow suspension). Failures email the repo owner.
