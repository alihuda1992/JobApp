...
**Claude integration**: [`mcp-server/`](mcp-server/) is a Model Context Protocol server that lets Claude (Claude Code or Claude Desktop) read and update the app conversationally — add jobs, move pipeline stages, score matches, draft cover letters — with changes appearing live in the open app. See [ARCHITECTURE.md](ARCHITECTURE.md) for how the pieces fit together.
